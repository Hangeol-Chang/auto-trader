...
class MACD_strategy(STRATEGY):
    ...
    def calculate_moving_averages(self):                
        ...
        # 이동평균 정렬 확인 (상승 추세: MA5 > MA20 > MA60)
        self.dataFrame['uptrend'] = (
            (self.dataFrame['MA5'] > self.dataFrame['MA20']) & 
            (self.dataFrame['MA20'] > self.dataFrame['MA60'])
        )
        
        # 이동평균 정렬 확인 (하락 추세: MA5 < MA20 < MA60)
        self.dataFrame['downtrend'] = (
            (self.dataFrame['MA5'] < self.dataFrame['MA20']) & 
            (self.dataFrame['MA20'] < self.dataFrame['MA60'])
        )
        
        # 골든 크로스: MA20이 MA60을 상향 돌파
        self.dataFrame['golden_cross'] = (
            (self.dataFrame['MA20_prev'] <= self.dataFrame['MA60_prev']) & 
            (self.dataFrame['MA20'] > self.dataFrame['MA60'])
        )
        
        # 데드 크로스: MA20이 MA60을 하향 돌파
        self.dataFrame['dead_cross'] = (
            (self.dataFrame['MA20_prev'] >= self.dataFrame['MA60_prev']) & 
            (self.dataFrame['MA20'] < self.dataFrame['MA60'])
        )
        
        # 단기 골든 크로스: MA5가 MA20을 상향 돌파
        self.dataFrame['short_golden_cross'] = (
            (self.dataFrame['MA5_prev'] <= self.dataFrame['MA20_prev']) & 
            (self.dataFrame['MA5'] > self.dataFrame['MA20'])
        )
        
        # 단기 데드 크로스: MA5가 MA20을 하향 돌파
        self.dataFrame['short_dead_cross'] = (
            (self.dataFrame['MA5_prev'] >= self.dataFrame['MA20_prev']) & 
            (self.dataFrame['MA5'] < self.dataFrame['MA20'])
        )
    ...
    def run(self, target_time=None) -> TradingSignal:
        ...
        short_golden_cross = latest['short_golden_cross']
        short_dead_cross = latest['short_dead_cross']
        ...
        # === 매수 신호 점수 계산 ===
        if golden_cross:  # 중장기 골든 크로스 (강력한 매수 신호)
            buy_score += 40
        
        if short_golden_cross and uptrend:  # 단기 골든 크로스 + 상승 추세
            buy_score += 30
        ...
        if ma5 > ma20 > ma60 and macd > macd_signal:  # 완전한 상승 배열
            buy_score += 15
        
        # === 매도 신호 점수 계산 ===
        if dead_cross:  # 중장기 데드 크로스 (강력한 매도 신호)
            sell_score += 40
        
        if short_dead_cross and downtrend:  # 단기 데드 크로스 + 하락 추세
            sell_score += 30
        ...
        if ma5 < ma20 < ma60 and macd < macd_signal:  # 완전한 하락 배열
            sell_score += 15
        
        # === 신호 결정 로직 ===
        if buy_score >= 50:  # 강력한 매수 신호
            confidence = min(0.9, 0.6 + (buy_score - 50) * 0.01)
            position_size = min(1.0, (buy_score - 30) * 0.02)
        ...
        elif sell_score >= 50:  # 강력한 매도 신호
            confidence = min(0.9, 0.6 + (sell_score - 50) * 0.01)
            position_size = min(1.0, (sell_score - 30) * 0.02)
